...
def main():
    ...
    argparser.add_argument('--threshold', dest='threshold', help='dep energy cut', default = 0.0, type=float)
        ...
        if fnmatch.fnmatch(infile, 'dataset_1_photons_tensor_no_pedding_euclidian_nentry12*.pt'):
            ...
            for shower in loaded_file[0]:
                ...
                if nshowers%2 == 0:
                    ...
                    mask = ~(shower[:,0] < threshold)
                    E_.extend( shower[:,0][mask]*GeV )
                    X_.extend( shower[:,1][mask] )
                    Y_.extend( shower[:,2][mask] )
                    Z_.extend( shower[:,3][mask] )
                nshowers+=1
    ...
    if transform == 1:
        ...
        print(f'Fitting transformation function for hit Z')
        transform_z = QuantileTransformer(output_distribution='normal').fit(Z_)
    ...
    for infile in os.listdir(indir):
        if fnmatch.fnmatch(infile, 'dataset_1_photons_tensor_no_pedding_euclidian_nentry*.pt'):
            ...
            if transform == 1:
                ...
                dump(transform_z, open(os.path.join(odir,'transform_z.pkl'), 'wb'))              
            # Check
